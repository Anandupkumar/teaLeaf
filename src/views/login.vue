            ...
            <div class="overlay-container">
               <div class="overlay">
                  <div class="overlay-left">
                     ...
                     <button class="invert" id="signIn" @click="toggle">
                        Log In
                     </button>
                  </div>
...
const checkOtp = async (data) => {
  const params = {
   ...
   const response = await Service.VerifyOtp(params);
   if (response.code == "200") {
      console.log(response);
   }
   // else if (response.code == '400') {
   //     this.$toast.show(response.message, { theme: 'outline', position: 'top-right', type: 'error', duration: 2000 });
   // } else if (response.code == "403") {
   //     this.$toast.show('Token expired please login again', { theme: 'outline', position: 'top-right', type: 'success', duration: 2000 });
...
#bg-page {
   ...
   align-items: center;
   background-size: cover; /* Adjust this to control how the image is scaled */
...
.container {
   ...
   .overlay {
      ...
      height: 100%;
      width: 200%;
      background: linear-gradient(to bottom right, #fff532, #fb2929);
      color: #000000;
